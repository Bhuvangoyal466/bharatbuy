                ...
                <div className="container px-5 py-24 mx-auto">
                    ...
                    <div className="text-center mb-16">
                        ...
                        <p className="text-gray-600 text-lg max-w-2xl mx-auto">
                            Decorate your world with our premium sticker
                            collection! Perfect for laptops, bottles, and more.
                        </p>
                    </div>
                    ...
                    {Object.keys(products).length === 0 && (
                        <div className="text-center py-20">
                            ...
                            <p className="text-gray-600 mb-8">
                                We're working on bringing you amazing stickers.
                                Check back soon!
                            </p>
                            <Link href="/">
        ...
        if (item.title in Stickers) {
            if (item.color && Array.isArray(item.color)) {
                item.color.forEach((color) => {
                    if (
                        !Stickers[item.title].color.includes(color) &&
                        item.availableQty > 0
                    ) {
                        Stickers[item.title].color.push(color);
                    }
                });
            }
            if (item.size && Array.isArray(item.size)) {
                item.size.forEach((size) => {
                    if (
                        !Stickers[item.title].size.includes(size) &&
                        item.availableQty > 0
                    ) {
            ...
            if (item.availableQty > 0) {
                Stickers[item.title].color = Array.isArray(item.color)
                    ? [...item.color]
                    : [item.color];
                Stickers[item.title].size = Array.isArray(item.size)
                    ? [...item.size]
                    : [item.size];
            } else {
